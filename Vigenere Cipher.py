...
keyword_indices = []
# find the index of each letter in keyword then add to list
for i in range(len(keyword)):
    keyword_indices.append(alphabet.find(keyword[i]))

index1 = 0
index2 = 0
# list for sum of message's and keyword's indices
sum_message_keyword = []
...
# if sum less than the len of alphabet, add to list, otherwise take mod then add to list
    if sum < len(alphabet):
        sum_message_keyword.append(sum)
    elif sum >= len(alphabet):
        sum_message_keyword.append(sum % len(alphabet))
    index1 += 1
...
# take the index from list of sum message and keyword to the variable alphabet
for p in range(len(sum_message_keyword)):
    mod_indices = alphabet[sum_message_keyword[p]]
